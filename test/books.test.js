...
describe('Books API', () => {
  test('GET /vibelibrary/books returns paginated books', async () => {
    const url = '/vibelibrary/books?page=1&limit=5';
    const res = await request(app).get(url);
    logger.append({ testFile: __filename, testName: 'GET /vibelibrary/books returns paginated books', input: { url }, output: { status: res.status, body: res.body } });
  ...
  test('Search books by title', async () => {
    const url = '/vibelibrary/books?search=Midnight';
    const res = await request(app).get(url);
  ...
  test('Search books by author name', async () => {
    const url = '/vibelibrary/books?author=Alice';
    const res = await request(app).get(url);
  ...
  test('Checkout and return flow', async () => {
    const listUrl = '/vibelibrary/books?search=Midnight';
    ...
    const checkoutUrl = `/vibelibrary/books/${id}/checkout`;
    ...
    const returnUrl = `/vibelibrary/books/${id}/return`;
  ...
  test('Create, update and delete a book', async () => {
    // Create
    const createUrl = '/vibelibrary/books';
    ...
    // Update
    const updateUrl = `/vibelibrary/books/${bookId}`;
    ...
    // Delete
    const deleteUrl = `/vibelibrary/books/${bookId}`;
    ...
    // Confirm deletion
    const getRes = await request(app).get(`/vibelibrary/books/${bookId}`);
    logger.append({ testFile: __filename, testName: 'Get deleted book', input: { url: `/vibelibrary/books/${bookId}` }, output: { status: getRes.status, body: getRes.body } });
    expect(getRes.status).toBe(404);
  });

  test('Create book with invalid payload returns 400', async () => {
    const url = '/vibelibrary/books';
  ...
  test('Update book with invalid payload returns 400', async () => {
    ...
    const createRes = await request(app).post('/vibelibrary/books').send({ title: 'TmpBook', pageCount: 10 });
    const id = createRes.body.id;
    const url = `/vibelibrary/books/${id}`;
  ...
  test('Delete non-existing book returns 404', async () => {
    const url = '/vibelibrary/books/999999';
